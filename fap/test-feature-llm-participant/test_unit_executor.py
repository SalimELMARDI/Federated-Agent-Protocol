...
async def _stub_llm_error(query: str) -> LLMResponse:
    del query
...
async def test_default_governance_applies_summary_only_prefix(monkeypatch: MonkeyPatch) -> None:
...
async def test_default_governance_records_internal_privacy_class(monkeypatch: MonkeyPatch) -> None:
...
async def test_public_raw_governance_exports_content_unchanged(monkeypatch: MonkeyPatch) -> None:
...
async def test_restricted_governance_collapses_to_vote_only(monkeypatch: MonkeyPatch) -> None:
...
async def test_sensitive_raw_request_is_downgraded_to_summary_only(monkeypatch: MonkeyPatch) -> None:
...
async def test_llm_failure_propagates_as_exception(monkeypatch: MonkeyPatch) -> None:
    """A failed LLM call must propagate LLMCallError instead of converting to success string.

    This makes LLM failures protocol-visible (HTTP 503) rather than treating them as
    successful task completions with error strings that leak into aggregates.
    """
    import pytest
    ...
    with pytest.raises(LLMCallError, match="connection refused"):
        await execute_task_create(_build_message())
...
async def test_source_ref_carries_model_name_as_source_id(monkeypatch: MonkeyPatch) -> None:
...
async def test_source_ref_carries_endpoint_as_source_path(monkeypatch: MonkeyPatch) -> None:
...
async def test_source_ref_title_includes_model_name(monkeypatch: MonkeyPatch) -> None:
...
async def test_source_refs_are_identical_on_task_complete_and_aggregate_submit(
...
async def test_aggregate_submit_provenance_ref_links_to_policy_attest(
    monkeypatch: MonkeyPatch,
) -> None:
...
async def test_aggregate_submit_summary_matches_task_complete_summary(
    monkeypatch: MonkeyPatch,
) -> None:
...
async def test_all_messages_carry_same_task_run_and_trace_ids(monkeypatch: MonkeyPatch) -> None:
