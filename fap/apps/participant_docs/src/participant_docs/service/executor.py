"""Deterministic participant_docs execution service using the local docs connector."""
...
from participant_docs.adapters.local_docs import LocalDoc, search_docs
...
DEFAULT_POLICY_REF = "policy.docs.v0"
VOTE_ONLY_SUMMARY = "[VOTE ONLY] No content exported"


class ParticipantExecutionResult(BaseModel):
    """Canonical result bundle returned by participant_docs execution."""
...
def execute_task_create(message: TaskCreateMessage) -> ParticipantExecutionResult:
    """Execute a task-create request against local docs and apply shared policy."""
...
def _build_local_content_and_source_refs(query: str) -> tuple[str, list[SourceRef]]:
    """Build a deterministic local result string and source refs from matching docs."""
    matches = search_docs(query)
    if not matches:
        return f"No relevant docs found for query: {query}", []

    titles = "; ".join(doc.title for doc in matches)
    return f"Matched docs: {titles}", _build_source_refs(matches)


def _build_source_refs(matches: list[LocalDoc]) -> list[SourceRef]:
    """Build deterministic source refs from matching local documents."""
    return [
        SourceRef(
            participant_id=PARTICIPANT_ID,
