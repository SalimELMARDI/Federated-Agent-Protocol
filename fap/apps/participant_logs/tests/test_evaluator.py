"""Tests for the participant_logs task-create evaluator."""
...
from participant_logs.service.capabilities import SUPPORTED_CAPABILITIES
from participant_logs.service.evaluator import evaluate_task_create
...
def test_accepts_when_all_requested_capabilities_are_supported() -> None:
    ...
    decision = evaluate_task_create(
        build_task_create_message(["logs.search", "logs.summarize"])
    )
...
def test_rejects_when_one_unsupported_capability_is_requested() -> None:
    ...
    decision = evaluate_task_create(
        build_task_create_message(["logs.search", "logs.translate"])
    )

    assert isinstance(decision, TaskRejectMessage)
    assert decision.payload.retryable is False
    assert "logs.translate" in decision.payload.reason
...
def test_rejects_when_multiple_unsupported_capabilities_are_requested() -> None:
    ...
    decision = evaluate_task_create(
        build_task_create_message(["logs.search", "logs.translate", "logs.classify"])
    )

    assert isinstance(decision, TaskRejectMessage)
    assert "logs.translate" in decision.payload.reason
    assert "logs.classify" in decision.payload.reason
...
def test_response_envelope_preserves_task_run_and_trace_ids() -> None:
    ...
    inbound = build_task_create_message(["logs.search"])
...
def test_response_envelope_sets_sender_recipient_and_message_type_correctly() -> None:
    ...
    inbound = build_task_create_message(["logs.search", "logs.unsupported"])

    decision = evaluate_task_create(inbound)

    assert decision.envelope.sender_id == "participant_logs"
    assert decision.envelope.recipient_id == inbound.envelope.sender_id
    assert decision.envelope.domain_id == "participant_logs"
