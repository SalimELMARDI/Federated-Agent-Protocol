"""Deterministic participant_logs execution service using the local logs connector."""
...
from participant_logs.adapters.local_logs import LocalLogEvent, search_logs
...
DEFAULT_POLICY_REF = "policy.logs.v0"
VOTE_ONLY_SUMMARY = "[VOTE ONLY] No content exported"


class ParticipantExecutionResult(BaseModel):
    """Canonical result bundle returned by participant_logs execution."""
...
def execute_task_create(message: TaskCreateMessage) -> ParticipantExecutionResult:
    """Execute a task-create request against local logs and apply shared policy."""
...
def _build_local_content_and_source_refs(query: str) -> tuple[str, list[SourceRef]]:
    """Build a deterministic local result string and source refs from matching log events."""
    matches = search_logs(query)
    if not matches:
        return f"No relevant log events found for query: {query}", []

    sources = "; ".join(event.source for event in matches)
    return f"Matched log events: {sources}", _build_source_refs(matches)


def _build_source_refs(matches: list[LocalLogEvent]) -> list[SourceRef]:
    """Build deterministic source refs from matching log events."""
    return [
        SourceRef(
            participant_id=PARTICIPANT_ID,
