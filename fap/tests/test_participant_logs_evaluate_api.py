"""Tests for the participant_logs evaluation endpoint."""
...
from participant_logs.main import app

client = TestClient(app)


def build_envelope(message_type: MessageType, *, sender_id: str, recipient_id: str) -> MessageEnvelope:
    """Return a valid envelope for participant_logs evaluate API tests."""
...
def test_evaluate_endpoint_returns_accept_for_supported_task_create() -> None:
    ...
    inbound = build_task_create_message(["logs.search", "logs.summarize"])
    ...
    assert body["envelope"]["sender_id"] == "participant_logs"
...
def test_evaluate_endpoint_returns_reject_for_unsupported_task_create() -> None:
    ...
    inbound = build_task_create_message(["logs.search", "logs.translate"])
...
def test_evaluate_endpoint_rejects_non_task_create_messages() -> None:
    ...
    assert response.json() == {
        "detail": {
            ...
            "message": "participant_logs can only evaluate 'fap.task.create' messages",
        }
    }
...
def test_evaluate_endpoint_rejects_malformed_known_kind_payload() -> None:
    ...
    raw_message = message_to_dict(build_task_create_message(["logs.search"]))
...
def test_health_endpoint_still_works() -> None:
    """Participant_logs health route should still be available."""
    response = client.get("/health")

    assert response.status_code == 200
    assert response.json() == {"status": "ok", "service": "participant_logs"}
