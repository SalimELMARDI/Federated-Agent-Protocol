"""Tests for the coordinator->participant_logs execute-dispatch API loop."""
...
def build_client(participant_app: FastAPI, *, database_path: Path) -> TestClient:
    """Return a fresh coordinator client wired to an in-process participant_logs app."""
...
def test_dispatch_execute_returns_three_canonical_messages(tmp_path: Path) -> None:
    ...
    client = build_client(create_participant_logs_app(), database_path=tmp_path / "coordinator.db")
    ...
    assert body["task_complete"]["payload"]["participant_id"] == "participant_logs"
    ...
    assert body["policy_attest"]["payload"]["participant_id"] == "participant_logs"
    ...
    assert body["aggregate_submit"]["payload"]["participant_id"] == "participant_logs"
...
def test_get_run_reflects_completed_participants_after_execute_dispatch(tmp_path: Path) -> None:
    ...
    assert completed_participants[0]["participant_id"] == "participant_logs"
    assert completed_participants[0]["status"] == "completed"
    assert completed_participants[0]["summary"] == "[SUMMARY ONLY] Matched log events: privacy-monitor"
    assert completed_participants[0]["message_id"]
    assert completed_participants[0]["source_refs"][0]["source_id"] == "log-002"
...
def test_get_run_reflects_policy_attestations_after_execute_dispatch(tmp_path: Path) -> None:
    ...
    assert policy_attestations[0]["participant_id"] == "participant_logs"
    assert policy_attestations[0]["policy_ref"] == "policy.logs.v0"
...
def test_get_run_reflects_aggregate_submissions_after_execute_dispatch(tmp_path: Path) -> None:
    """The run snapshot should include participant-originated aggregate submissions."""
    client = build_client(create_participant_logs_app(), database_path=tmp_path / "coordinator.db")
    create_message = build_task_create_message()
    ...
    assert aggregate_submissions[0]["participant_id"] == "participant_logs"
    assert aggregate_submissions[0]["contribution_type"] == "summary"
    assert aggregate_submissions[0]["summary"] == "[SUMMARY ONLY] Matched log events: privacy-monitor"
    assert aggregate_submissions[0]["provenance_ref"]
    assert aggregate_submissions[0]["message_id"]
    assert aggregate_submissions[0]["source_refs"][0]["source_id"] == "log-002"
...
def test_dispatch_execute_for_unknown_run_returns_run_not_found(tmp_path: Path) -> None:
    ...
    response = client.post(f"/runs/{unknown_run_id}/dispatch/participant-logs/execute")
...
def test_downstream_non_200_becomes_participant_execution_failed(tmp_path: Path) -> None:
    ...
    client.post("/messages", json=message_to_dict(create_message))
    response = client.post(f"/runs/{create_message.envelope.run_id}/dispatch/participant-logs/execute")

    assert response.status_code == 502
    assert response.json() == {
        "detail": {
            "code": "participant_execution_failed",
            "message": "participant_logs execution failed with status 503",
        }
    }
...
def test_downstream_malformed_response_becomes_invalid_participant_execution_response(tmp_path: Path) -> None:
    ...
    assert response.json() == {
        "detail": {
            ...
            "message": "participant_logs returned invalid JSON response",
        }
    }
