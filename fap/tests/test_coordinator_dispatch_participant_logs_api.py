"""Tests for the coordinator->participant_logs dispatch API loop."""
...
def build_task_create_message(requested_capabilities: list[str]) -> TaskCreateMessage:
    ...
    return TaskCreateMessage(
        ...
        payload=TaskCreatePayload(
            ...
            description="Retrieve deterministic log events for coordinator review.",
...
def test_dispatch_supported_create_returns_canonical_task_accept(tmp_path: Path) -> None:
    ...
    create_message = build_task_create_message(["logs.search", "logs.summarize"])

    create_response = client.post("/messages", json=message_to_dict(create_message))
    dispatch_response = client.post(
        f"/runs/{create_message.envelope.run_id}/dispatch/participant-logs"
    )
    ...
    assert body["payload"]["participant_id"] == "participant_logs"
    assert body["payload"]["accepted_capabilities"] == ["logs.search", "logs.summarize"]
...
def test_dispatch_unsupported_create_returns_canonical_task_reject(tmp_path: Path) -> None:
    ...
    client = build_client(create_participant_logs_app(), database_path=tmp_path / "coordinator.db")
    create_message = build_task_create_message(["logs.search", "logs.translate"])
    ...
    assert "logs.translate" in body["payload"]["reason"]
...
def test_get_run_reflects_accepted_participant_after_dispatch(tmp_path: Path) -> None:
    ...
    client = build_client(create_participant_logs_app(), database_path=tmp_path / "coordinator.db")
    create_message = build_task_create_message(["logs.search"])

    client.post("/messages", json=message_to_dict(create_message))
    client.post(f"/runs/{create_message.envelope.run_id}/dispatch/participant-logs")
    ...
    assert run_response.json()["accepted_participants"] == ["participant_logs"]
    ...
    assert run_response.json()["rejected_participants"] == [
        {
            ...
            "reason": "Unsupported capabilities requested: logs.translate",
            "retryable": False,
        }
    ]
...
def test_dispatch_for_unknown_run_returns_run_not_found(tmp_path: Path) -> None:
    ...
    response = client.post(f"/runs/{unknown_run_id}/dispatch/participant-logs")
...
def test_downstream_non_200_becomes_participant_evaluation_failed(tmp_path: Path) -> None:
    ...
    client.post("/messages", json=message_to_dict(create_message))
    response = client.post(f"/runs/{create_message.envelope.run_id}/dispatch/participant-logs")

    assert response.status_code == 502
    assert response.json() == {
        "detail": {
            "code": "participant_evaluation_failed",
            "message": "participant_logs evaluation failed with status 503",
        }
    }
