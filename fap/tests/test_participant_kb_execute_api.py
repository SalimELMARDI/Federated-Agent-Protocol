"""Tests for the participant_kb execution endpoint."""
...
def build_envelope(
    ...
    *,
    ...
    """Return a valid envelope for participant_kb execute API tests."""
...
def test_execute_endpoint_returns_three_canonical_messages_with_default_governance() -> None:
    ...
    assert body["task_complete"]["payload"]["summary"] == "[SUMMARY ONLY] Matched KB entries: Privacy controls"
    ...
    assert body["policy_attest"]["envelope"]["sender_id"] == "participant_kb"
    assert body["policy_attest"]["envelope"]["recipient_id"] == inbound.envelope.sender_id
    assert body["policy_attest"]["envelope"]["domain_id"] == "participant_kb"
    ...
    assert body["aggregate_submit"]["envelope"]["sender_id"] == "participant_kb"
    assert body["aggregate_submit"]["envelope"]["recipient_id"] == inbound.envelope.sender_id
    assert body["aggregate_submit"]["envelope"]["domain_id"] == "participant_kb"
...
def test_execute_endpoint_public_raw_returns_raw_export() -> None:
    ...
    assert response.status_code == 200
    assert response.json()["task_complete"]["payload"]["summary"] == "Matched KB entries: Privacy controls"
...
def test_execute_endpoint_rejects_non_task_create_messages() -> None:
    ...
    assert response.json() == {
        "detail": {
            ...
            "message": "participant_kb can only execute 'fap.task.create' messages",
        }
    }
