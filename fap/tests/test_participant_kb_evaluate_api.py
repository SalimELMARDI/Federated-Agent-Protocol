"""Tests for the participant_kb evaluation endpoint."""
...
def build_envelope(message_type: MessageType, *, sender_id: str, recipient_id: str) -> MessageEnvelope:
    """Return a valid envelope for participant_kb evaluate API tests."""
...
def test_evaluate_endpoint_returns_accept_for_supported_task_create() -> None:
    ...
    inbound = build_task_create_message(["kb.lookup", "kb.summarize"])
    ...
    assert body["envelope"]["sender_id"] == "participant_kb"
...
def test_evaluate_endpoint_returns_reject_for_unsupported_task_create() -> None:
    ...
    inbound = build_task_create_message(["kb.lookup", "kb.translate"])
...
def test_evaluate_endpoint_rejects_non_task_create_messages() -> None:
    ...
    assert response.json() == {
        "detail": {
            ...
            "message": "participant_kb can only evaluate 'fap.task.create' messages",
        }
    }
...
def test_evaluate_endpoint_rejects_malformed_known_kind_payload() -> None:
    ...
    raw_message = message_to_dict(build_task_create_message(["kb.lookup"]))
